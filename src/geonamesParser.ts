...
export async function extractCitiesData(allTheCountriesFile: string, destinationFolder: string, batchSize = 25): Promise<void> {
	...
	for (let i = 0; i < countryList.length; i += batchSize) {
		...
		await processCountries(batchCountries, allTheCountriesFile, destinationFolder);
	}
}
...
export async function processCountries(countries: string[], allTheCountriesFile: string, destinationFolder: string): Promise<void> {
	...
	const processLine = (line: string): void => {
		...
		// Join altNames2 with altName1 and remove all empty strings
		const searchNames = [asciiname, ...alternatenames.split(",")].filter(n => n);

		const city: City = {
			id: parseInt(id),
			name: name.trim(),
			search: searchNames,
			Long: parseFloat(long) as Longitude,
